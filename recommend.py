...
def recommend(product,detail,important_size,important_category_name,dog_data):
    
    # 현재 가슴, 등, 목 길이 기준으로 코딩
    # 목은 모두 범위 데이터
    clothes_data_json = json.loads(product)
    clothes_data_detail_json = json.loads(detail)
    # important_size 기준 사이즈
    ...
    if important_size == "size_chest":
        if important_category_name=="하네스":
            if nodata_part[0]==False:
                input = 1
            else :
                input = 2

        elif important_category_name=="상의" and important_category_name=="원피스":
            if nodata_part[0]==False or nodata_part[2]==False:
                input = 1
            else :
                input = 3
        elif important_category_name=="하의":
    ...
    if input==1:
        ...
        # 강아지 정보
        for zero in range(6,10):
            if dog_data[zero] == '':
                dog_data[zero] = float(-1)

        dog_size_chest = float(dog_data[6])
        dog_size_neck = float(dog_data[7])
        dog_size_back = float(dog_data[8])
        dog_size_leg = float(dog_data[9])
        ...
        # 마진 값
        margin = clothes_data_json.get("product_size_margin")
